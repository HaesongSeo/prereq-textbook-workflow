...
last_approved: ""
next_action: "Project Config를 채우고 Phase 0을 시작한다"
chapters_approved: []
sections_approved: []
---
...
> 상태 블록(맨 위)이 담지 못하는 것을 적는다 — 왜 거기서 멈췄는가, 정독 순서,
> 재설명 금지 목록. `phase`와 승인된 장 목록을 여기에 다시 적지 않는다.

- 현재 Phase: **Phase 0** (논문 확보 및 분석)
- 마지막 승인 지점:
- 다음 작업:
- 대기 중인 질문:
- **K2 이상 (재설명 금지):**
...
**만들 때 반드시 다른 문헌의 인용을 걸러낸다.** 걸러내지 않으면 인용 문헌의
`Theorem 5`가 원문의 `Lemma 5`와 충돌한다고 보고한다. **거짓 양성이 나오는
검사기는 곧 무시당하므로, 이 걸러내기가 검사기의 생사를 가른다.**

- `\cite[...]{...}`는 **키가 목표 논문일 때만** 판정한다.
- 산문을 훑기 전에 **모든 `\cite[...]{...}`를 지운다.** 안 지우면 다른 문헌의
  위치 지시가 본문처럼 읽힌다.
